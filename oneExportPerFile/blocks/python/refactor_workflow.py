...
def run_one_export_workflow(cfg: WorkflowConfig) -> Path:
    ...
    starting_branch = get_current_branch(repo_root)
    _vlog(cfg, f"Detected starting branch: {starting_branch}")
    state = create_state(cfg.state_dir, repo_root, cfg.target_dir, starting_branch)
    _vlog(cfg, f"State file created: {state.file_path}")

    try:
        operation_branch = create_branch(repo_root, starting_branch, f"operation-{cfg.target_dir}")
        ...
        try:
            checkout_branch(repo_root, starting_branch)
            _vlog(cfg, f"Restored starting branch: {starting_branch}")
        except Exception as exc:
            error_msg = f"Failed to restore starting branch '{starting_branch}': {exc}"
            _vlog(cfg, error_msg)
            state.add_error(error_msg)
            state.save()
