...
def merge_branch(target_wt: Path, source: str, source_wt: Path | None = None, cleanup: bool = True) -> str:
    """Merge *source* branch into the branch checked out at *target_wt*.

    Uses ``--no-ff`` to preserve history.  If *source_wt* is provided and
    *cleanup* is True the source worktree is removed after a successful merge.
    Returns HEAD sha after merge.
    """
    run_cmd(
        ["git", "merge", "--no-ff", source, "-m", f"Merge {source}"],
        cwd=target_wt,
    )
    sha = run_cmd(["git", "rev-parse", "HEAD"], cwd=target_wt)
    if source_wt is not None and cleanup:
        run_cmd(["git", "worktree", "remove", "--force", str(source_wt)], cwd=target_wt)
    return sha
